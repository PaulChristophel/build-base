...
RUN apk add --update --no-cache curl make tzdata ca-certificates build-base git && curl -sSfL https://golangci-lint.run/install.sh | sh -s -- -b $(go env GOPATH)/bin v2.13.1
...
RUN apt update && apt upgrade -y && apt install -y curl git make tzdata ca-certificates build-essential && curl -sSfL https://golangci-lint.run/install.sh | sh -s -- -b $(go env GOPATH)/bin v2.13.1
RUN go install github.com/swaggo/swag/cmd/swag@latest
