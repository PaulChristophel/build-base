...
FROM --platform=amd64 node:23.7.0-alpine3.20 AS base-node-musl
RUN npm install -g pnpm
FROM --platform=amd64 golang:1.23.6-alpine3.20 AS musl
...
# Create the base build image
FROM --platform=amd64 node:23.7.0-bookworm AS base-node-glibc
RUN npm install -g pnpm
FROM --platform=amd64 golang:1.23.6-bookworm AS glibc
...
.UN go install github.com/swaggo/swag/cmd/swag@latest
